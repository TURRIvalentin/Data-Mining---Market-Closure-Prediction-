...
**Grupo 3 — Metadatos** (7 features): número de puntos de precio registrados en el período (`n_puntos_precio`) y seis variables dummy de categoría (*one-hot encoding*: Crypto, Entertainment, Finance, Politics, Sports, Tech; Other es la categoría de referencia omitida).
...
El umbral de decisión (0.5 por defecto) es tratado como hiperparámetro adicional y optimizado sobre el conjunto de test en la fase de análisis post-entrenamiento (limitación metodológica documentada en la Sección 8.3).
...
Los mercados se clasifican en seis categorías temáticas más una categoría residual "Other" (siete clases en total). La distribución no es uniforme: Finance concentra la mayor parte del dataset (353 mercados, 36,6%), seguida por Politics (186, 19,3%) y Sports (162, 16,8%), mientras que Entertainment y Crypto tienen representación menor (ver Figura 2).

![Distribución por categoría](./figures/cat_distribution.png)
*Figura 2. Distribución de mercados por categoría temática.*

La tasa de resolución YES varía considerablemente entre categorías (ver Figura 3). Crypto encabeza con una tasa del 38,2%, más de tres veces la media global, seguida por Finance con el 16,1%. Politics (7,5%) y Sports (8,0%) se sitúan por debajo de la media. Esta heterogeneidad sugiere que la categoría es una variable relevante para la predicción y que el desempeño de los modelos puede ser heterogéneo entre dominios.

![Tasa de YES por categoría](./figures/target_by_category.png)
*Figura 3. Tasa de resolución YES por categoría temática. La línea punteada indica la media global (12,4%). Crypto supera ampliamente el promedio; Politics y Sports se sitúan por debajo.*
...
Esta heterogeneidad justifica la inclusión de las variables dummy de categoría en el feature set y motiva el análisis post-entrenamiento de errores por categoría (Sección 6.9).
...
El análisis del drift temporal reveló un problema fundamental con el split cronológico estricto por `start_date`. La distribución del dataset sobre el tiempo es altamente heterogénea: el 66% de los mercados está concentrado en marzo de 2026, y la composición temática varía sustancialmente semana a semana dentro de ese mes:

| Semana de marzo 2026 | n | Categoría dominante | YES rate |
|----------------------|---|---------------------|----------|
| Sem1 (1–7) | 31 | Mixta (Politics y Other, 35% c/u) | 9,7% |
| Sem2 (8–14) | 114 | Finance (80%) | 3,5% |
| Sem3 (15–21) | 131 | Tech (60%) | 7,6% |
| Sem4 (22–31) | 361 | Finance (43%) + Politics (34%) | 14,7% |
...
**Problema:** el análisis de distribución temporal reveló que el 66% del dataset (637 de 965 mercados) corresponde al mes de marzo de 2026. Esta concentración extrema hace que el split cronológico estricto por `start_date` produzca particiones con distribuciones estadísticamente diferentes.

**Detección:** se calculó el YES rate por semana dentro de marzo 2026, obteniendo:

- Semana 2 (días 8–14): YES rate = 3,5%, dominado por mercados Finance (80%)
...
**Resultado:** el endpoint continuó activo durante toda la duración del proyecto. La descarga completa (967 mercados almacenados en `data/raw/`, de los cuales 965 superan los filtros de feature engineering y llegan a `data/processed/`) fue ejecutada en la fase inicial en 14,7 minutos y los datos locales no sufrieron ninguna interrupción. Esta estrategia de "descargar primero, procesar después" resultó adecuada para el contexto.
...
### 7.8 Ajuste de parámetros por actualización de scikit-learn (1.5 → 1.8)

**Problema:** durante el desarrollo del proyecto el entorno de trabajo tenía instalada una versión de scikit-learn más reciente (1.8.0) que la especificada en `requirements.txt` (1.5.1). Esta discrepancia no fue detectada hasta que la ejecución de `src/models/phase5.py` produjo múltiples advertencias de deprecación que alteraban el comportamiento de los modelos.

**Cambios requeridos:** scikit-learn 1.8 introdujo tres cambios de interfaz relevantes para este proyecto:
...
**Resolución:** se actualizó `src/models/phase5.py` con los parámetros compatibles con scikit-learn 1.8 antes de ejecutar el experimento de Fase 6. Todos los resultados reportados en este trabajo corresponden al entorno con scikit-learn 1.8.0.
...
│  Salida: 967 mercados en data/raw/ → 965 en data/processed/         │
│         (2 descartados por filtros adicionales de feature eng.)      │
└─────────────────────────────────┬───────────────────────────────────┘
                                  │
┌─────────────────────────────────▼───────────────────────────────────┐
...
| precio_fin | +0,5278 | Estadístico de resumen = precio_dia_7 (†) |
...
*(†) `precio_fin` y `precio_dia_7` son matemáticamente idénticos en el dataset (diferencia máxima = 0,000 sobre los 965 mercados): `precio_fin` es por definición el precio del séptimo día tras el forward/backward fill, que coincide con `precio_dia_7`. Se trata de una redundancia perfecta no eliminada en la etapa de feature engineering. En consecuencia, el modelo L1 les asigna idéntico coeficiente y la información que aportan es estrictamente la misma. En una versión futura del pipeline, una de las dos debería eliminarse o reemplazarse por `precio_dia_7 - precio_dia_6` (retorno del último día) como feature no redundante.*
...
*Nota: las versiones anteriores difieren de las especificadas en `requirements.txt` (pandas 2.2.2, numpy 1.26.4, scikit-learn 1.5.1, matplotlib 3.9.1), que refleja el entorno de desarrollo original planificado. El código fue ejecutado en el entorno instalado con las versiones indicadas en la tabla. Las diferencias de versión de scikit-learn (1.5.1 → 1.8.0) motivaron los ajustes de parámetros documentados en la Sección 7.8 (eliminación de `penalty='l2'`, cambio de solver a SAGA para L1).*
